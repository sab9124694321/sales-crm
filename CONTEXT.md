# SZB CRM — Полный контекст проекта (v2.3.0)
...
- `export_inn.php` — экспорт ИНН (обновлён: добавлена дата актуальности списка, менеджеры могут отмечать «Установлен»)
- **`tips_rollout.php`** — модуль «Раскатка чаевых» (добавлен в v2.3.0)
- `leads.php` — лиды
- `quests.php` — квесты
- `terms.php` — условия
...
-- tips_rollout — раскатка чаевых
CREATE TABLE tips_rollout (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    type TEXT NOT NULL CHECK(type IN ('personal', 'command')),
    tb TEXT DEFAULT 'СЗБ',
    public_uuid TEXT,
    tid TEXT NOT NULL,
    uuid TEXT,
    ...
    sn_kkt TEXT,
    link TEXT,
    previously_connected INTEGER DEFAULT 0,
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    user_id INTEGER NOT NULL,
    user_name TEXT,
...
check_history — история массовых проверок (используется в export_inn.php)
...
9. Новый модуль: Раскатка чаевых (v2.3.0)

Страница tips_rollout.php позволяет менеджерам и руководителям вводить данные для подключения чаевых на терминалах.

Формы

Командные чаевые: ТБ, UUID команды, TID терминала.
Персональные чаевые: TID (головной), UUID команды чаевых, IMEI1, СН ККТ (стикер), признак «Подключены ранее» (Да/Нет). Ссылка генерируется автоматически.
Фильтры

По типу (командные/персональные), ТБ, UUID, TID, IMEI1, СН ККТ, подключены ранее, пользователь, дате.
Выгрузка

Две кнопки: «Выгрузить командные» и «Выгрузить персональные» – создают CSV-файлы с соответствующими данными (в формате Excel с разделителем ; и BOM).
Права доступа

Выгрузка доступна всем, кроме менеджеров (manager). Редактирование записей доступно всем авторизованным пользователям.
Таблица tips_rollout

Хранит все введённые записи с датой создания, user_id и user_name.

10. Обновления в export_inn.php (v2.3.0)

Менеджеры могут отмечать чекбокс «Установлен» (is_installed). Остальные чекбоксы (checked_performance, checked_turnover, is_target) доступны только начальникам.
Добавлена дата актуальности списка при массовой проверке. Поле reference_date позволяет указать, на какую дату применять проверку. Эта дата записывается в check_history.performed_at и используется в отчёте термена (RR пересчитывается с учётом этой даты).
11. История изменений (ключевые)

v2.3.0 (2026-09-03)

Новый модуль: «Раскатка чаевых» (tips_rollout.php).
Новая таблица: tips_rollout для хранения данных по чаевым.
Обновление export_inn.php: менеджеры могут отмечать «Установлен», добавлена дата актуальности списка.
Обновление навигации: добавлена ссылка на «Чаевые» в дашборд и другие страницы.
Обновление CONTEXT.md до актуальной версии.
v2.2.1 (2026-07-11)
...
Чаевые — дополнительная услуга, подключаемая на терминал
Модуль "Я звоню"
...
BACKUP_DIR="/root/backups/deploy_$(date +%Y%m%d_%H%M%S)"
mkdir -p $BACKUP_DIR
docker cp sales-crm:/var/www/html $BACKUP_DIR/html_backup
docker cp sales-crm:/var/www/html/sales.db $BACKUP_DIR/sales.db.backup
docker cp sales-crm:/var/www/html/config.php $BACKUP_DIR/config.php.backup
docker cp sales-crm:/var/www/html/db.php $BACKUP_DIR/db.php.backup
docker exec -it sales-crm bash -c "cd /var/www/html && git pull origin main && chown www-data:www-data sales.db && chmod 664 sales.db && echo '=== ГОТОВО ==='"
...
14. Памятка для следующей сессии
...
Последнее обновление: 2026-09-03 (v2.3.0)
