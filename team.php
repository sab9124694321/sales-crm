...
$client_type = $_GET['client_type'] ?? 'all'; // new, expansion, all
...
// ------------------------------------------------------------------
// Функция получения данных из inn_records (включая expected_turnover и client_type)
function getProductSumsFromInn($pdo, $tabel, $date_from, $date_to, $key_type, $pirate_type, $client_type) {
    ...
    $sql = "
        SELECT 
            ...
            COALESCE(SUM(expected_turnover), 0) as expected_turnover_sum
        FROM inn_records
    ...
    if ($hasKey) {
        if ($key_type === 'key') $sql .= " AND is_key = 1";
        elseif ($key_type === 'nonkey') $sql .= " AND is_key = 0";
    }
    if ($hasStation) {
        if ($pirate_type === 'pirate') $sql .= " AND station_type = 'pirate'";
        elseif ($pirate_type === 'newreg') $sql .= " AND station_type = 'newreg'";
    }
    if ($hasClient && $client_type !== 'all') {
        $sql .= " AND client_type = '" . addslashes($client_type) . "'";
    }
...
function getAggregatedMetrics($user_ids, $pdo, $selected_month, $date_from, $date_to, $days_passed, $days_left, $work_days_total) {
    global $key_type, $pirate_type, $client_type;
    $total_plans = [
    ...
    foreach ($user_ids as $uid) {
        ...
        // Продукты из inn_records
        $prod = getProductSumsFromInn($pdo, $tabel, $date_from, $date_to, $key_type, $pirate_type, $client_type);
        ...
        $prodToday = getProductSumsFromInn($pdo, $tabel, $date_to, $date_to, $key_type, $pirate_type, $client_type);
        foreach (['registrations','smart_cash','pos_systems','inn_leads'] as $f) {
...
elseif ($user_role == 'terman') {
    ...
    $territories = $stmt->fetchAll();
    if ($level == 'root') $show_territories = true;
    elseif ($level == 'territory') {
        $stmt = $pdo->prepare("SELECT id, tabel_number, full_name, role FROM users WHERE role IN ('head','territory_head') AND territory_id = ?");
        $stmt->execute([$selected_id]);
        $managers = $stmt->fetchAll();
...
function renderMetricsGrid($metrics, $title = '', $days_passed = 0) {
    ...
    foreach ($metrics as $m) {
        ...
        if ($m['unit'] == '₽') $avg_per_day_formatted = number_format($avg_per_day, 2, '.', ' ');
        else $avg_per_day_formatted = number_format($avg_per_day, 1, '.', ' ');
        ...
        echo "<div class='metric-value-row'><span class='metric-value $status'>$today_fact</span><span class='metric-daily-target' title='Дневная цель'>$daily_target</span></div>";
        ...
        if ($label === 'Дозвоны' && $aiCalls > 0) echo "<div class='metric-sub' style='color:#0d6efd;'>🤖 AI-звонки: $aiCalls</div>";
        echo "</div>";
    }
    echo '</div>';
}
...
// ------------------------------------------------------------------
// Таблица сотрудников (добавлена колонка expected_turnover)
function renderEmployeesTable($employees, $pdo, $days_passed, $days_left, $selected_month, $date_from, $date_to, $work_days_total) {
    ...
    $allColumns = [
        ...
        'rko' => '🏦 РКО',
        'expected_turnover' => '💳 Оборот по терминалам'
    ];
    ...
    foreach ($employees as $emp) {
        ...
        if (!$plans) {
            $plans = [
                ...
                'teams_plan'=>3, 'turnover_plan'=>1500000, 'rko_plan'=>0, 'expected_turnover_plan'=>16000000
            ];
        }
        ...
        $factM['expected_turnover'] = $prod['expected_turnover_sum'];
        ...
        $factD['expected_turnover'] = $prodToday['expected_turnover_sum'];
        ...
        foreach ($visibleArray as $col) {
            ...
            if (in_array($col, ['turnover','rko','expected_turnover'])) $avg_per_day = number_format($avg_per_day, 2, '.', ' ');
...
foreach ($team_members as $m) {
    ...
    foreach ($calendar_days as $date_str) {
        $prod_day = getProductSumsFromInn($pdo, $tab, $date_str, $date_str, $key_type, $pirate_type, $client_type);
        $val = $prod_day['registrations'] + $prod_day['smart_cash'] + $prod_day['pos_systems'];
        $turnover = $prod_day['expected_turnover_sum'];
    ...
    foreach ($prev_months as $ym) {
        ...
        if ($from <= $to) {
            $prod_prev = getProductSumsFromInn($pdo, $tab, $from, $to, $key_type, $pirate_type, $client_type);
            $val = $prod_prev['registrations'] + $prod_prev['smart_cash'] + $prod_prev['pos_systems'];
            $turnover = $prod_prev['expected_turnover_sum'];
        } else {
            $val = 0;
            $turnover = 0;
        }
...
<div class="nav-links">
    ...
    <form class="filter-form-inline" method="GET" action="team.php">
        ...
        <select name="client_type">
            <option value="all" <?= $client_type=='all'?'selected':'' ?>>Все типы клиентов</option>
            <option value="new" <?= $client_type=='new'?'selected':'' ?>>Новый</option>
            <option value="expansion" <?= $client_type=='expansion'?'selected':'' ?>>Расширение</option>
        </select>
        <button type="submit">🔍 Смотреть</button>
    </form>
</div>
