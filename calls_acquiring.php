...
<head>
    ...
    <style>
        ...
        .cheat-sheet { background: linear-gradient(135deg,#1a5c3a,#0d3b1f); color:#fff; border-radius:16px; padding:16px; margin-bottom:16px; }
        .cheat-sheet h3 { font-size:14px; margin-bottom:10px; color:#a0d4b0; }
        .cheat-sheet .script-line { font-size:13px; margin:6px 0; padding:6px 10px; background:rgba(255,255,255,0.08); border-radius:8px; }
        .cheat-sheet .script-line .num { color:#4ade80; font-weight:bold; margin-right:6px; }
        .cheat-sheet-toggle { background:#1a5c3a; color:#fff; border:none; padding:10px 16px; border-radius:10px; cursor:pointer; font-size:13px; font-weight:600; margin-bottom:10px; }
        .cheat-sheet-content { display:none; }
        .cheat-sheet-content.open { display:block; }
        .score-panel { display:grid; grid-template-columns:1fr 1fr; gap:8px; margin-top:10px; }
        .score-item { background:rgba(255,255,255,0.08); border-radius:8px; padding:8px; text-align:center; font-size:11px; }
        .score-item .val { font-size:18px; font-weight:800; color:#4ade80; }
        .score-item .lbl { color:#a0a0c0; }
    </style>
</head>
<body>
<div class="container">
    ...
    <!-- ШПАРГАЛКА ПОД РУКОЙ -->
    <button class="cheat-sheet-toggle" onclick="toggleCheatSheet()">📋 Показать шпаргалку по эквайрингу</button>
    <div id="cheatSheet" class="cheat-sheet cheat-sheet-content">
        <h3>🎯 ЭКВАЙРИНГ СБЕР — быстрая подсказка</h3>
        <div class="score-panel">
            <div class="score-item"><div class="val">1%</div><div class="lbl">по картам<br>3 мес</div></div>
            <div class="score-item"><div class="val">0,3%</div><div class="lbl">по QR / FaceScan<br>6 мес</div></div>
            <div class="score-item"><div class="val">50%</div><div class="lbl">компенсация<br>НДС</div></div>
            <div class="score-item"><div class="val">1,6%</div><div class="lbl">минимум<br>после акции</div></div>
        </div>
        <div style="margin-top:12px; font-size:11px; color:#a0a0c0;">
            Тинькофф мин. 1,9% · Райффайзен от 1,8% · ВТБ от 1,7%
        </div>
        <div class="script-line"><span class="num">1</span>«<strong>[Имя]</strong>, добрый день. Меня зовут <strong>[Имя]</strong>, Сбербанк. У вас <strong>30 секунд</strong>?»</div>
        <div class="script-line"><span class="num">2</span>«Вопрос в одно предложение: вы уже принимаете карты и <strong>какой сейчас банк</strong>?»</div>
        <div class="script-line"><span class="num">3</span>«Смотрите: <strong>1%</strong> на карты, <strong>0,3%</strong> по QR, компенсируем <strong>половину НДС</strong>. Это <strong>[Y]₽</strong> экономии. Могу за 2 минуты посчитать точнее.»</div>
        <div class="script-line"><span class="num">4</span>«<strong>Понимаю.</strong> Давайте сравним вашу текущую ставку с нашей?»</div>
        <div class="script-line"><span class="num">5</span>«Давайте так: я сейчас посчитаю и перезвоню <strong>[сегодня в 18:00]</strong>. Или удобнее в WhatsApp?»</div>
        <div style="margin-top:10px; font-size:11px; color:#fbbf24;">
            ❌ Не говори: «лучший», «низкие ставки», «расскажу преимущества», «вы заинтересованы»<br>
            ✅ Говори: цифры, сравнение, «экономия [сумма]₽», «давайте сравним», конкретное время
        </div>
    </div>
...
// ============ ПРОМПТЫ ДЛЯ AI-КОУЧА (ЭКВАЙРИНГ СБЕР) ============
const COACH_PROMPT = `Ты — старший менеджер по продажам эквайринга Сбера. Ты слушаешь разговор новичка с клиентом.

Ключевые цифры Сбера:
- 1% на оплату картами (первые 3 месяца)
- 0,3% на оплату через QR и FaceScan (первые 6 месяцев)
- Компенсация 50% НДС по эквайрингу
- Минимальная ставка после акции — 1,6%
- Конкуренты: Тинькофф мин. 1,9%, Райффайзен от 1,8%, ВТБ от 1,7%

Проверь по пунктам и дай КРАТКИЙ совет (2-3 предложения максимум):

1. ПРИВЕТСТВИЕ — назвал ли менеджер имя клиента, сказал "Сбербанк", попросил 30 секунд (не "удобно ли")?
2. ВОВЛЕЧЕНИЕ — узнал ли текущий банк и ставку клиента ПЕРЕД тем, как начать продавать?
3. ПРЕЗЕНТАЦИЯ — назвал ли конкретные цифры (1%, 0,3%, НДС), перевёл ли проценты в рубли экономии?
4. ВОЗРАЖЕНИЯ — не спорил ли, не говорил ли "у нас лучше"? Перевёл ли в сравнение цифр?
5. ЗАВЕРШЕНИЕ — назвал ли конкретное время перезвона? Предложил ли WhatsApp?

Если менеджер всё делает правильно — похвали конкретно. Если есть ошибка — скажи, что именно и как сказать лучше. Не более 3 предложений.`;

const FINAL_ANALYSIS_PROMPT = `Ты — руководитель отдела продаж эквайринга Сбера. Проанализируй сеанс телефонных звонков менеджера.

Ключевые цифры Сбера: 1% по картам (3 мес), 0,3% по QR/FaceScan (6 мес), компенсация 50% НДС, минимум 1,6% после акции. Конкуренты: Тинькофф мин. 1,9%, Райффайзен от 1,8%.

Проанализируй:
1. Сколько звонков было по скрипту (приветствие → вовлечение → презентация цифр → работа с возражениями → завершение с конкретным временем)
2. Какие пункты скрипта пропущены чаще всего
3. Какие возражения встречались и как менеджер на них отвечал
4. Дай 2-3 конкретные рекомендации по улучшению

Будь мотивирующим, но конкретным. Не общие фразы — только actionable советы.`;

const CALL_SUMMARY_PROMPT = `Ты — эксперт по продажам эквайринга. Проанализируй этот один телефонный звонок.

Ключевые цифры Сбера: 1% по картам (3 мес), 0,3% по QR (6 мес), компенсация 50% НДС.

Оцени:
- Было ли приветствие с именем и "Сбербанк"?
- Узнал ли менеджер текущий банк клиента?
- Назвал ли конкретные цифры (1%, 0,3%, НДС)?
- Перевёл ли проценты в рубли экономии?
- Как отработал возражения (если были)?
- Было ли завершение с конкретным временем?

Дай краткую оценку (1-2 предложения) и 1 конкретный совет.`;
...
function toggleCheatSheet() {
    ...
    const btn = document.querySelector('.cheat-sheet-toggle');
    sheet.classList.toggle('open');
    btn.textContent = sheet.classList.contains('open') ? '📋 Скрыть шпаргалку' : '📋 Показать шпаргалку по эквайрингу';
}
